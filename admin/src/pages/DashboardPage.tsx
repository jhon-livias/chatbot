...
function ConvItem({
  ...
  return (
    ...
    >
      ...
      >
        {phoneInitials(conv.phoneNumber)}
  ...
  return (
    <div className="dash-chat">
      <header className="dash-chat-header">
        ...
        >
          {phoneInitials(meta?.phoneNumber ?? id)}
        </div>
        <div className="dash-chat-header-info">
          <span className="dash-chat-header-name">{meta?.phoneNumber ?? '…'}</span>
          <span className="dash-chat-header-sub">WhatsApp · {meta?.mode === 'human' ? 'Atención humana' : 'Bot'}</span>
        </div>
